...
using a simple API key header. Follows the same pattern as tools/integrations/wiza.py.
...
def apollo__search_people(
    ...
    data = resp.json()
    people_raw = data.get("people") or data.get("contacts") or []
    people = [_pick(p, _PERSON_SEARCH_FIELDS) for p in people_raw]
